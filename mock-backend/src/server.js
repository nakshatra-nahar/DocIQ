...
router.render = (req, res) => {
  if (req.url === "/feedback") {
    ...
    if (id === 'creative')
      res.status(200).json({
        "content": "You are a dociq, friendly and helpful AI assistant by Arc53 that provides help with documents. You give thorough answers with code examples if possible."
      })
    else if (id === 'strict') {
      res.status(200).json({
        "content": "You are an AI Assistant, dociq, adept at offering document assistance. \nYour expertise lies in providing answer on top of provided context."
      })
    }
    else {
      res.status(200).json({
        "content": "You are a helpful AI assistant, dociq, specializing in document assistance, designed to offer detailed and informative responses."
      })
    }
  }
  else {
